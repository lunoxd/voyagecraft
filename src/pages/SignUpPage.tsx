...
export const SignUpPage: React.FC = () => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (res.success && res.data) {
      ...
      setErrorMsg(res.error || 'Registration failed. Please check connectivity.');
    }
  };
    ...
    try {
      ...
      if (!res.success) {
        setTimeout(() => {
          ...
          addLog("AUTH-SERVICE", "SUCCESS", `New user profile initialized via Google OAuth.`);
          navigate('/packages');
        }, 600);
      }
    } catch (e: any) {
    ...
    <div className="max-w-md mx-auto py-12 space-y-8 font-sans">
      <div className="text-center space-y-3">
        ...
        <h1 className="text-3xl sm:text-4xl font-black tracking-tight text-black uppercase">
          {isAuthenticated ? "Session Active" : "Create Account"}
        </h1>
        <p className="text-sm text-neutral-600">
          {isAuthenticated
            ? "You are already authenticated with a VoyageCraft account."
            : "Join VoyageCraft to book curated expeditions with instantaneous seat reservation."}
        </p>
      </div>
