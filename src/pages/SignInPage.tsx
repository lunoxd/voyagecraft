...
export const SignInPage: React.FC = () => {
  ...
  const handleSignIn = async (e: React.FormEvent) => {
    ...
    if (res.success && res.data) {
      ...
      addLog("AUTH-SERVICE", "SUCCESS", `User ${res.data.email} successfully authenticated against SQL database.`);
      navigate('/packages');
    } else {
      // Fallback local signin if offline
      setCurrentUserRole(role);
      addLog("AUTH-SERVICE", "INFO", `Local authentication session started for ${email} (${role}).`);
      navigate('/packages');
    }
  };
      ...
      <div className="p-8 rounded-3xl bg-neutral-50/90 border border-neutral-200/80 shadow-lg shadow-black/5 space-y-6">
        ...
        <form onSubmit={handleSignIn} className="space-y-4 text-xs font-sans">
          ...
          <div className="space-y-2 pt-2 border-t border-neutral-200/80">
            <div className="text-[11px] font-semibold text-neutral-500 font-mono">Quick Demo Accounts:</div>
            <div className="flex flex-wrap gap-1.5">
              <button
                type="button"
                onClick={() => {
                  setEmail('alexander.wright@voyagecraft.internal');
                  setRole('ADMIN');
                }}
                className="px-3 py-1 rounded-full bg-white border border-neutral-200 text-neutral-800 text-[10px] font-mono hover:bg-neutral-100 cursor-pointer font-medium shadow-2xs"
              >
                Alexander (Admin)
              </button>
              <button
                type="button"
                onClick={() => {
                  setEmail('elena.rostova@voyagecraft.internal');
                  setRole('AGENT');
                }}
                className="px-3 py-1 rounded-full bg-white border border-neutral-200 text-neutral-800 text-[10px] font-mono hover:bg-neutral-100 cursor-pointer font-medium shadow-2xs"
              >
              ...
              <button
                type="button"
                onClick={() => {
                  setEmail('marcus.vance@voyagecraft.internal');
                  setRole('TRAVELER');
                }}
                className="px-3 py-1 rounded-full bg-white border border-neutral-200 text-neutral-800 text-[10px] font-mono hover:bg-neutral-100 cursor-pointer font-medium shadow-2xs"
              >
