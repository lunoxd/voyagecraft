...
function AutoBind(self: any, { include, exclude }: { include?: string[]; exclude?: string[] } = {}) {
  ...
  const filter = (key: string | symbol) => {
    ...
    const match = (pattern: string) => pattern === key;
  ...
  for (const key of getAllProperties(self.constructor.prototype)) {
    if (key === 'constructor' || !filter(key)) continue;
    const descriptor = Reflect.getOwnPropertyDescriptor(self.constructor.prototype, key);
...
class Media {
  ...
  constructor({ gl, geometry, scene, screen, viewport, image, length, index, planeWidth, planeHeight, distortion }: MediaOptions) {
    this.extra = 0;
    this.gl = gl;
...
class Canvas {
  ...
  planeGeometry: any;
  medias: Media[] = [];
  screen: { width: number; height: number } = { width: 1, height: 1 };
  ...
  constructor({ container, canvas, items, planeWidth, planeHeight, distortion, scrollEase, cameraFov, cameraZ }: CanvasOptions) {
  ...
  onTouchDown(e: MouseEvent | TouchEvent) {
    ...
    this.start = 'touches' in e && e.touches ? e.touches[0].clientY : (e as MouseEvent).clientY;
  }

  onTouchMove(e: MouseEvent | TouchEvent) {
    if (!this.isDown) return;
    const y = 'touches' in e && e.touches ? e.touches[0].clientY : (e as MouseEvent).clientY;
...
export const FlyingPosters: React.FC<FlyingPostersProps> = ({
  ...
  return (
    <div ref={containerRef} className={`posters-container ${className}`.trim()} style={style} {...props}>
